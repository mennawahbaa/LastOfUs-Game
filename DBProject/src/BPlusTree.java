...
public class BPlusTree implements java.io.Serializable{
	...
	public void delete(Comparable key ,String value) {
		Hashtable<String,Integer>bucket =this.search(key);
		if (bucket==null) {
			return ;
			}
		if (bucket.get(value)>1) {
			bucket.put(value,bucket.get(value)-1);
//			this.delete(key);
//			this.insert(key, bucket);
			}
		else {
			bucket.remove(value);
	...
	//updateValue
	public void update (Comparable key ,String oldval ,String newval) {
		if(oldval.equals(newval))
			return ;
		if (this.search(key)!=null)
			this.delete(key, oldval);
		this.insert(key, newval);
	}
	
	public void updateKey(Comparable oldKey ,Comparable newKey ,String value) {
	 ...
	 */
	public void insert(Comparable key, Hashtable<String,Integer>  value){
		if (isEmpty()) {
	...
	public void insert(Comparable key ,String value) {
		Hashtable<String,Integer> bucket = this.search(key);
		if (bucket==null) {
			bucket=new Hashtable<String,Integer>();
			bucket.put(value, 1);
			this.insert(key, bucket);
		}
		else {
...
//			this.insert(key, bucket);
	 ...
	 */
	public Hashtable<String,Integer> search(Comparable key) {
	 ...
	 * @return an ArrayList<Hashtable<String,Integer>> that holds all values of dictionary pairs
	 * whose keys are within the specified range
	 */
	public ArrayList<Hashtable<String,Integer>> search(Comparable lowerBound, Comparable upperBound) {

		// Instantiate Double array to hold values
		ArrayList<Hashtable<String,Integer>> values = new ArrayList<>();
	...
	public ArrayList<Hashtable<String,Integer>> searchStrictlyGreater(Comparable lowerBound, Comparable upperBound) {
	...
	public ArrayList<Hashtable<String,Integer>> searchStrictlyLess(Comparable lowerBound, Comparable upperBound) {
	...
	public Comparable minimumKey() {
		LeafNode currNode = this.firstLeaf;
		if (currNode == null) {
			return null;
		}
		return currNode.dictionary[0].key;
	}
	
	public Comparable maximumKey() {
		LeafNode currNode = this.firstLeaf;
		if (currNode == null) {
			return null;
		}
		...
		return currNode.dictionary[currNode.numPairs-1].key;
	}
	 ...
	 */
 class InternalNode extends Node implements java.io.Serializable { // TAGHYEEEEEEEEER
		int maxDegree;
		int minDegree;
		int degree;
	...
	public class LeafNode extends Node implements java.io.Serializable {
		...
		public LeafNode(int m, DictionaryPair dp) {
			this.maxNumPairs = m - 1; //hanel3ab hena
	...
	public class DictionaryPair implements Comparable<DictionaryPair>,java.io.Serializable{
		Comparable key;
		Hashtable<String,Integer> value;
		 ...
		 */
		public DictionaryPair(Comparable key, Hashtable<String,Integer> value2) {
			this.key = key;
			this.value = value2;
		}
	...
	public static void main(String[] args) {
		...
		BPlusTree b = new BPlusTree(5);
		String t = new String("bdd");
		String t2= new String("heba");
...
//		for (int i = 0; i < b.root.childPointers.length; i++) {
//			for (int j = 0; j < b.m; j++) {
//				System.out.print(((LeafNode)(b.root.childPointers[i])).dictionary[j]+ ", ");
//			}
//			System.out.print("    ");
//		}
		System.out.println(b.maximumKey());
		//b.update("Roba", t, t2);
		//System.out.println(b.search("Roba"));
//		System.out.println(b.search("Roba"));
//		b.delete("Roba", t);
//		System.out.println(b.search("Roba"));
//		b.delete("Roba", t2);
